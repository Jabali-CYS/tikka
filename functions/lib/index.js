 ...
 */
exports.validateAndPlaceOrder = (0, https_1.onCall)(async (request) => {
    ...
    if (!Array.isArray(items) || items.length === 0) {
        throw new https_1.HttpsError("invalid-argument", "Order must contain at least one valid menu item.");
    }
    ...
    try {
        return await db.runTransaction(async (transaction) => {
            ...
            for (const item of items) {
                if (!item.productId || typeof item.quantity !== "number" || item.quantity <= 0) {
                    throw new https_1.HttpsError("invalid-argument", "Invalid format for standard item parameters.");
                }
                const productRef = db.collection("products").doc(item.productId);
                const productDoc = await transaction.get(productRef);
                if (!productDoc.exists || productDoc.data()?.isDeleted === true) {
                    throw new https_1.HttpsError("failed-precondition", `No active menu record was found for product: ${item.productId}`);
                }
                ...
                if (sideId && typeof sideId === "string" && sideId.trim() !== "") {
                    const sideOptions = prodData.sideOptions || [];
                    const sideMatch = sideOptions.find((o) => o.id === sideId);
                    if (sideMatch) {
                        itemSum += parseFloat(sideMatch.price || "0");
                    }
                    else {
                        throw new https_1.HttpsError("failed-precondition", `Chosen side add-on '${sideId}' does not belong to product '${item.productId}'.`);
                    }
                }
            ...
            if (fulfillmentType === "delivery" && zoneId) {
                ...
                if (!zoneDoc.exists) {
                    throw new https_1.HttpsError("failed-precondition", "Specified delivery zone is unrecognized.");
                }
            ...
            if (couponCode) {
                ...
                if (couponSnap.empty) {
                    throw new https_1.HttpsError("not-found", "Invalid or deleted coupon code.");
                }
                ...
                if (expiry.getTime() < Date.now()) {
                    throw new https_1.HttpsError("failed-precondition", "The entered coupon has expired.");
                }
                ...
                if (maxLimit > 0 && currentCount >= maxLimit) {
                    throw new https_1.HttpsError("failed-precondition", "Coupon campaign limit reached.");
                }
                ...
                if (subtotal < minCouponOrder) {
                    throw new https_1.HttpsError("failed-precondition", `Coupon require minimum active purchase of: ${minCouponOrder} JOD.`);
                }
        ...
        throw new https_1.HttpsError("internal", error.message || "An error occurred compiling order ticket.");
    }
});
/**
 ...
 */
exports.awardLoyaltyPoints = (0, firestore_2.onDocumentUpdated)("orders/{orderId}", async (event) => {
 ...
 */
exports.redeemLoyaltyReward = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
    ...
    if (!selectedReward) {
        throw new https_1.HttpsError("invalid-argument", "The selected reward program code was not found inside catalogs.");
    }
    const userRef = db.collection("users").doc(uid);
    try {
        return await db.runTransaction(async (transaction) => {
            const userDoc = await transaction.get(userRef);
            if (!userDoc.exists) {
                throw new https_1.HttpsError("not-found", "Loyalty customer user account record was not found.");
            }
            const currentPoints = parseInt(userDoc.data()?.points || "0");
            if (currentPoints < selectedReward.cost) {
                throw new https_1.HttpsError("failed-precondition", `Insufficient points. Required: ${selectedReward.cost}, Your balance: ${currentPoints}.`);
            }
        ...
        throw new https_1.HttpsError("internal", error.message || "Failed to redeem loyalty points reward.");
    }
});
/**
...
exports.validateCoupon = (0, https_1.onCall)(async (request) => {
    ...
    if (!couponCode) {
        throw new https_1.HttpsError("invalid-argument", "Coupon code parameter is required.");
    }
    const couponQuery = db.collection("coupons")
        .where("code", "==", couponCode)
        .where("isDeleted", "==", false)
    ...
    if (couponSnap.empty) {
        throw new https_1.HttpsError("not-found", "The entered coupon code was not recognized.");
    }
    const couponDoc = couponSnap.docs[0];
    const couponData = couponDoc.data();
    // Expired Verification
    const expiry = new Date(couponData.expiryDate);
    if (expiry.getTime() < Date.now()) {
        throw new https_1.HttpsError("failed-precondition", "This discount coupon has expired.");
    }
    // Global Usage Limits Check
    const maxLimit = parseInt(couponData.usageLimit || "0");
    const currentCount = parseInt(couponData.usedCount || "0");
    if (maxLimit > 0 && currentCount >= maxLimit) {
        throw new https_1.HttpsError("failed-precondition", "This coupon has reached its maximum global usage limit.");
    }
    ...
    if (couponData.createdForUid && couponData.createdForUid !== uid) {
        throw new https_1.HttpsError("permission-denied", "This promotional voucher is restricted to another customer account.");
    }
    ...
    const usageCheck = await db.collection("coupon_redemptions")
        ...
        .get();
    if (!usageCheck.empty) {
        throw new https_1.HttpsError("already-exists", "This coupon has already been redeemed on your account.");
    }
    // Verify subtotal
    const minOrder = parseFloat(couponData.minOrder || "0");
    if (subtotal < minOrder) {
        throw new https_1.HttpsError("failed-precondition", `Minimum order of ${minOrder} JOD is required for this code. Current subtotal: ${subtotal} JOD.`);
    }
    return {
        code: couponCode,
...
exports.seedDatabase = (0, https_1.onCall)(async (request) => {
    ...
    if (data.key !== "grill_tikka_seed_2026") {
        throw new https_1.HttpsError("permission-denied", "Unauthorized seed request.");
    }
